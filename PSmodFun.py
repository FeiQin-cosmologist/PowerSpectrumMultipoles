...
def PijFun(parm,PT,MU,pstyp):
    if(pstyp=='auto'):
        f, b1,b2,bs,b3nl, sigma_v1,sigma_v2,sigma_v3,sigma8rat = parm
    if(pstyp=='crs'):  
        f, b1,b2,bs,b3nl, b1v,b2v,bsv,b3nlv, sigma_v1,sigma_v2,sigma_v3, sigmav_v1,sigmav_v2,sigmav_v3,sigma8rat = parm  
    k  =PT[0,0:] ; Pl =PT[1,0:]   
    ...
    Ks11=PT[32,0:];K20=PT[33,0:];Ks20=PT[34,0:];K30=PT[35,0:];Ks30=PT[36,0:];  sig3_squre=PT[37,0:];
    ...
    for ss in range(len(MU)):
        mu=MU[ss]
        if(pstyp=='auto'):
            P00=b1*b1 *sigma8rat *(Pl+2.*sigma8rat*(I00+3.*k*k*Pl*J00))+ (b1+b1)*sigma8rat**2*(b2*K00+bs*Ks00+b3nl*sig3_squre*Pl)+sigma8rat**2*(1./2.*b2*b2*K01+1./2.*bs*bs*Ks01+b2*bs*Ks02)
            ...
            P02=f*f*b1*sigma8rat**2*(I02+mu**2*I20+2.*k*k*Pl*(J02+mu**2*J20))-f*f*k*k*(sigvL_squre+sigma_v1 )* P00-f*f*sigma8rat**2*(b2*(K20+mu**2*K30)+bs*(Ks20+mu**2*Ks30))
            P03=-f*f*k*k*(sigvL_squre+sigma_v2)*P01
            P04=-1./2.*f*f*f*f*b1*k*k*(sigvL_squre+sigma_v3)*sigma8rat**2*(I02+mu**2*I20+2.*k*k*Pl*(J02+mu**2*J20))+1./4.*f*f*f*f*b1*b1*k**4*P00*((sigvL_squre+sigma_v3)**2+sig4_squre )
            P11=f*f*sigma8rat*(mu**2*(Pl+sigma8rat*(2.*I11+(b1+b1+b1+b1)*I22+b1*b1*I13+6.*k**2*Pl*(J11+(b1+b1)*J10)))+b1*b1*sigma8rat*I31)
            P12=f*f*f*sigma8rat**2*(I12+mu**2*I21-b1*(I03+mu**2*I30)+2.*k**2*Pl*(J02+mu**2*J20))-f*f*k*k*(sigvL_squre+sigma_v1 )* P01
            P13=-f*f*k*k* f*f*sigma8rat*(    (sigvL_squre+sigma_v2)*mu**2*(Pl+sigma8rat*(2.*I11+(b1+b1+b1+b1)*I22+6.*k*k*Pl*(J11+(b1+b1)*J10)))+ (sigvL_squre+sigma_v1 )*b1*b1*sigma8rat*(mu**2*I13+I31) )
            P22=1./4.*f*f*f*f*sigma8rat**2*(I23+2.*mu**2*I32+mu**4*I33)+f*f*f*f*k**4*(sigvL_squre+sigma_v1 )**2 *P00-f*f*k*k*(sigvL_squre+sigma_v1 )*(2.*P02-f*f*sigma8rat**2*(b2*(K20+mu**2*K30)+bs*(Ks20+mu**2*Ks30)))            
        if(pstyp=='crs'): 
            P00=b1*b1 *sigma8rat *(Pl+2.*sigma8rat*(I00+3.*k*k*Pl*J00))+ (b1+b1)*sigma8rat**2*(b2*K00+bs*Ks00+b3nl*sig3_squre*Pl)+sigma8rat**2*(1./2.*b2*b2*K01+1./2.*bs*bs*Ks01+b2*bs*Ks02)
            P01=f*b1  *sigma8rat *(Pl+2.*sigma8rat*(I01+b1*I10+3.*k*k*Pl*(J01+b1*J10))-b2*sigma8rat*K11-bs*sigma8rat*Ks11)-f*sigma8rat**2*(b2*K10+bs*Ks10+b3nl*sig3_squre*Pl)
            P02=f*f*b1*sigma8rat**2*(I02+mu**2*I20+2.*k*k*Pl*(J02+mu**2*J20))-f*f*k*k*(sigvL_squre+sigma_v1 )* P00-f*f*sigma8rat**2*(b2*(K20+mu**2*K30)+bs*(Ks20+mu**2*Ks30))
...
def Pk_MOD(params,sigma8_fid,PT,ps_type, Growthz ):
    ...
    fsigma8,b1sigma8,b2sigma8,    sigma_v  = params 
    
    b1vsigma8,b2vsigma8 = b1sigma8, b2sigma8
    sigma_v1 ,sigma_v2  = sigma_v , sigma_v  
    sigmav_v1,sigmav_v2 = sigma_v , sigma_v  
    sigma_v3, sigmav_v3 = sigma_v1, sigmav_v1  
    ...
    if ('den' in ps_type):
        P_00,P_01,P_02,P_03,P_04,P_11,P_12,P_13,P_22 =PijFun( [f, b1,b2,bs,b3nl, sigma_v1,sigma_v2,sigma_v3,sigma8rat ],PT,mu, 'auto')
    ...
    if ('mom' in ps_type):
        P_00,P_01,P_02,P_03,P_04,P_11,P_12,P_13,P_22 =PijFun( [f, b1v,b2v,bsv,b3nlv, sigmav_v1,sigmav_v2,sigmav_v3,sigma8rat ],PT,mu, 'auto')
    ...
    if ('crs' in ps_type):
        P_00,P_01,P_02,P_03,P_04,P_11,P_12,P_13,P_22 =PijFun( [f, b1,b2,bs,b3nl, b1v,b2v,bsv,b3nlv, sigma_v1,sigma_v2, sigma_v3,sigmav_v1,sigmav_v2,sigmav_v3,sigma8rat],PT,mu, 'crs')
        P_crs  = (1.0e2/PT[0,0:]*((mu*(P_01 + P_02 + P_11 + mu**2*(3.0/2.0*P_03 + 2.0*P_04 + 2.0*P_12 + 3.0*P_13 + 1.0/5.0*P_22))).T)).T
...
def CHI2(params,    ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type ):
    ...
    return chi_squared
def tPDF(params,    Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type ):
    chi_squared=CHI2(params,    ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type ) 
...
def lnprior(params):
    ...
    if(0.  <b2sigma8<3.):pdf=1.
    else:return -np.inf
    if( 0.<sigma_v<250.):pdf=1.
    else:return -np.inf
    return 0.0
# 5.1: the post of MCMC:
def lnpost_tPDF(params    ,   Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type  ):
    prior = lnprior(params )
    if not np.isfinite(prior):
        return -np.inf
    like = tPDF(params ,Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type )
    return prior - like
